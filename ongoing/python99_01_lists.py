...
Based on the 99 Prolog Problems (https://sites.google.com/site/prologsite/prolog-problems).
'''
...
def compress(l):
	return [item for i,item in enumerate(l) if i+1 == len(l) or item != l[i+1] ]
# Test
...
def pack(l):
	...
	return packed
# Test
assert pack(l4) == [['a','a','a','a'],['b'],['c','c'],['a','a'],['d'],['e','e','e','e']]
...
# Test
assert encode(l4) == [[4,'a'],[1,'b'],[2,'c'],[2,'a'],[1,'d'],[4,'e']]

'''1.11 (*) Modified run-length encoding.
	Modify the result of problem 1.10 in such a way that if an element has no duplicates it is simply copied into the result list. Only elements with duplicates are transferred as [N,E] terms.'''
# Solution
encode_modified = lambda l: [([len(a),a[0]] if len(a) != 1 else a[0]) for a in pack(l4) if a != []]
# Test
assert encode_modified(l4) == [[4,'a'],'b',[2,'c'],[2,'a'],'d',[4,'e']]
